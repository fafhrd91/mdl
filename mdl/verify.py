...
def _verify(iface, candidate, tentative=0, vtype=None):
    ...
    if not tentative and not tester(candidate):
        raise DoesNotImplement((iface, candidate))
    ...
    for name, desc in iface.namesAndDescriptions(1):
        ...
        if isinstance(attr, FunctionType):
            if sys.version[0] == '3' and isinstance(candidate, type):
                ...
                meth = fromFunction(attr, iface, name=name)
        elif name == '__call__' and isinstance(candidate, FunctionType):
            meth = fromFunction(candidate, iface, name=name)
        elif (isinstance(attr, MethodTypes) and
...
def _incompat(required, implemented):
    ...
    if ((len(implemented['positional']) < len(required['positional'])) and
